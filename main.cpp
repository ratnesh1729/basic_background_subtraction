...
{
    Mat kernel = getStructuringElement(MORPH_ELLIPSE,cv::Size(3,3)); // an elliptical structure element which is used to process each pixel
...
//Option 1: mean image based subtraction: Although median will be more robust, but options 2, 3 (MOG) is a better way to remove BG
void remove_mean_image_from_video(const string inp, const string output_folder, const float thresh = 20, bool disp = false)
{
    Mat mean_img;
    ...
    for(int i=0; i<num_frames; i++) //for a video a while loop can be used
    {
        ...
        threshold( gray, dst, thresh, 255,0 ); //thresh for thresholding: An important parameter, which is nevertheles difficult to set
...
//compute MOG based FG model : DOXYGEN format can be used for input/output. I am just ignoring now because its simple enough
void simple_mog_based_fg(const string inp, const MOG_params &params, const string output_folder, const bool disp = false)
{
    VideoCapture capture(inp);
    Ptr<BackgroundSubtractor> MOG; //automatic destruction is called (smart pointer, deletion based on RefCount)
    MOG = new BackgroundSubtractorMOG(params.history,params.nmixtures, params.bgR, params.noiseSigma); //MOG : You can also use MOG2 Z.Zivkovic
    //
    Mat frame, fg;
    const int num_frames = capture.get(CV_CAP_PROP_FRAME_COUNT);
    const int width = capture.get(CV_CAP_PROP_FRAME_WIDTH);
...
void mog_based_fg(const string inp, const MOG_params &params, const string output_folder, const bool disp = false)
{
    ...
    for(int i=0; i< num_frames; i++) //for a video a while loop can be used
    {
        ...
        if(params.bShadowDetection) //shadows are marked with 127, so if shadow detection is used, then I should remove 127 from mask
        {
            //shadow =127 in mask
            //make this to zero
...
void cimg_processing_3D_data(const string inp, const string opt, const bool disp =false, const bool morph = false, const int sigma =5 , const int thresh = 10)
{
    // This is an elementary step to background removal: It will only work is movement is fast and motion is perpendicular to optical axis
    // I keep this function to show you a simple line removal (time)

    // This function is slightly memory intensive: Can be made more efficient by dropping the volume image and loading again when needed.
    VideoCapture capture(inp);
    const int num_frames = capture.get(CV_CAP_PROP_FRAME_COUNT);
    const int width = capture.get(CV_CAP_PROP_FRAME_WIDTH);
    const int height = capture.get(CV_CAP_PROP_FRAME_HEIGHT);
    // init a Cimg Image
    UCCimg volume(width, height,num_frames,3,0); //3 for default channels: I can put a check, but we're in colorful world :-)
    for(int i=0; i< num_frames; i++) //for a video a while loop can be used
    {
        Mat frame;
        capture >> frame; // capture the current frame
    ...
    UCCimg vol_img = volume.get_deriche(sigma,order,'z'); // a memory intensive step. Ideally I should do it on the object
...
{
    // How to use
    cimg_usage("Default arguments are provided: Please choose -h while running and choose approprite values for parameters");
    //string inp = cimg_option("-input", "/home/fratnesh/Dropbox/img_small/imgs/%05d.jpg", "Input Folder Location for Image Files");
    string inp = cimg_option("-input", "/home/fratnesh/Dropbox/img_small/%05d.jpg", "Input Folder Location for Image Files");
    string opt = cimg_option("-output", "/home/fratnesh/opt", "Input Folder Location for Image Files");
    const int option = cimg_option("-bg_option", 2, "1- mean image subtraction, 2- MOG, 3- MOG by Zoran, 4- temporal gradient ");
    ...
    const float lR = cimg_option("-lR", 0.005, "learning rate for MOG: 0 means only first frame as BG i.e. no learning");
    bool is_morph_proc = cimg_option("-morph", true, "whether to perform morphological processing" );
    const bool is_shadow_det = cimg_option("-shadow_det", false, "Shadow detection for option 3"); //shadows are 127 in mask
